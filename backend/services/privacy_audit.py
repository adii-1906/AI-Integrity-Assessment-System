...
DATASET VERSION - Validated on Enron Email Dataset (Kaggle)
Same class name, same method .audit(), same return structure as original.
"""
...
class PrivacyAuditor:
    """
    Same interface as original PrivacyAuditor.
    12-class PII classifier validated on Enron Email Dataset.
    """

    PII_PATTERNS = [
        PIIPattern("email", r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b',
                   "medium", 0.5, "Email address"),
        PIIPattern("phone_us", r'\b(?:\+1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b',
                   "medium", 0.5, "US phone number"),
        PIIPattern("phone_international", r'\+[1-9]\d{1,14}',
                   "medium", 0.5, "International phone number"),
        PIIPattern("phone_indian", r'\b(?:\+91[-.\s]?)?[6-9]\d{9}\b',
                   "medium", 0.5, "Indian phone number"),
        PIIPattern("ssn", r'\b\d{3}[-\s]?\d{2}[-\s]?\d{4}\b',
                   "critical", 0.95, "Social Security Number"),
        PIIPattern("aadhaar", r'\b[2-9]\d{3}[-\s]?\d{4}[-\s]?\d{4}\b',
                   "critical", 0.95, "Aadhaar number"),
        PIIPattern("credit_card", r'\b(?:\d{4}[-\s]?){3}\d{4}\b',
                   "critical", 0.98, "Credit card number"),
        PIIPattern("passport", r'\b[A-Z]{1,2}\d{6,9}\b',
                   "high", 0.8, "Passport number"),
        PIIPattern("ip_address", r'\b(?:\d{1,3}\.){3}\d{1,3}\b',
                   "low", 0.3, "IP address"),
        PIIPattern("date_of_birth", r'\b(?:0[1-9]|1[0-2])[-/](?:0[1-9]|[12]\d|3[01])[-/](?:19|20)\d{2}\b',
                   "medium", 0.6, "Date of birth"),
        PIIPattern("bank_account", r'\b\d{9,18}\b(?=.*(?:account|routing|bank))',
                   "critical", 0.95, "Bank account number"),
        PIIPattern("medical_record", r'\b(?:MRN|medical record|patient id)[-:\s]*\d+\b',
                   "high", 0.85, "Medical record number"),
    ]
    ...
    def audit(self, text: str) -> Dict[str, Any]:
        """
        Same signature as original PrivacyAuditor.audit().
        ...
        # 1. Pattern-based PII detection (12 classes)
        ...
        # 3. Score (matches original logic exactly)
        if findings:
            weighted_risk = (
